...
if (isset($_POST['Edit'])) {
    ...
    $sql_total_bobot = "SELECT SUM(bobot_kriteria) as total_bobot FROM bobot_data_kriteria WHERE id_kriteria != '$id_kriteria'";
    $result_total_bobot = mysqli_query($conn, $sql_total_bobot);
    $row_total_bobot = mysqli_fetch_assoc($result_total_bobot);
    $total_bobot = $row_total_bobot['total_bobot'];

    // Hitung total bobot setelah diubah
    $new_total_bobot = $total_bobot - $r['bobot_kriteria'] + $bobot_kriteria;

    if ($new_total_bobot >= 100) {
        echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Total bobot kriteria tidak boleh melebihi 100</div>';
    } else {
        // Proses pengeditan data
        $a = mysqli_query($conn, "UPDATE bobot_data_kriteria SET nama_kriteria='$nama_kriteria', bobot_kriteria='$bobot_kriteria' WHERE id_kriteria='$id_kriteria'");
        if ($a) {
            echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Data Berhasil Di Simpan.</div>';
            echo "<meta http-equiv='refresh' content='2; url=data-kriteria.php'>";
        } else {
